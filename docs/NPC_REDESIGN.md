...
>
> **BASELINE OF RECORD RE-PINNED AT T-131 (2026-07-31)** to
...
1. **Grade at 1,000 seeds × 120 days, not 100 — DONE (R0b).** The 100-seed arm cannot resolve
   ...
   re-run bigger.**
   > **Baseline of record is `docs/balance/baseline-t131-explore-ap.json`** (1,000 seeds ×
   > 120 days × 8 policies = 8,000 runs, re-pinned at T-131 2026-07-31 — owner ruling D1
   > repriced Explore bands 3-4 from calendar days to extra dice, which moved the
   > `explorer`, `smuggler` and `fleet` rows, so `baseline-t125-hangout` no longer
   > describes HEAD).
