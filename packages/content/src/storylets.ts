...
// T-114 · the storylet ids an explore `questline` outcome row can schedule.
// DERIVED from the rows in `exploration.ts`, never transcribed. `exploration.ts`
// imports nothing from this module at runtime (its `StoryletEffects` import is
// `import type` and is erased), so this edge is acyclic.
import { EXPLORE_SCHEDULED_STORYLET_IDS } from './exploration.js';
...
export const STORYLETS = defineStorylets(
  [
      ...
      choices: [
        {
          ...
          prose:
            'Promise Doc you will keep a channel open and burn a reply through the beacon net.',
          effects: {
    ...
    // --- Mandatory 9: the plain per-system port beats (reachability-critical) ---
    {
      ...
      choices: [
        ...
        },
        {
          ...
          prose:
            'Their grain, their fight. Sign your gantry slip and leave the exchange to itself.',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      choices: [
        ...
        },
        {
          ...
          prose:
            'Tell the navigator your heading is set. She frowns, but finds another hull to ask.',
        },
      ],
    },
    {
    ...
    // --- T-114 · The explore questline hooks (docs/EXPLORE_REDESIGN.md §2.2/§2.5)
    //
    // THREE BEATS SCHEDULED BY A CONTENT ROW RATHER THAN BY A CHOICE. An explore
    // `questline` outcome carries `{ storyletId, delayDays }` and the engine turns
    // it into the SAME `StoryletEffects.schedule` a played choice produces, through
    // the same `applyEffects` — so these are ordinary scheduled episodes and need
    // no machinery of their own. What is new is only WHO scheduled them, which is
    // why `defineStorylets` now takes `EXPLORE_SCHEDULED_STORYLET_IDS` (see the
    // parameter's own header in storyletValidation.ts).
    //
    // Each is `scheduledOnly` with NO further gate. That is deliberate: an era or
    // system gate on a beat whose scheduler is a chance find would strand the
    // schedule entry the day the gate never opened, and §2.5 puts a questline row's
    // conditions on the STORYLET rather than on the row precisely so they can be
    // seen here. Each carries a `wireResolution` so an unplayed hook is resolved by
    // the existing dusk sweep (`resolveAbandonedChains`) instead of dangling —
    // verified safe: that sweep reads only `entry.storyletId` and `entry.dueDay`,
    // never `sourceStoryletId`, so an explore-sourced entry cannot confuse it.
    {
      id: 'explore.cold-berth.survivor',
      title: 'The Berth That Was Warm',
      prose:
        'The survivor you cut out of the cold berth has slept two days and talked for one. Her ship was not lost where the registry says it was lost, and she can put a finger on the chart to prove it — if someone will carry her far enough to matter.',
      repeat: 'never',
      trigger: {
        scheduledOnly: true,
      },
      wireResolution: {
        graceDays: 7,
        wireMessage:
          'The survivor pulled off the cold berth signed onto another hull while your channel stayed dark, and took her chart finger with her.',
        effects: {
          flags: [{ name: 'explore.cold-berth.resolved', value: 'wire' }],
        },
      },
      choices: [
        {
          ...
          label: 'Hear her out',
          prose:
            'Sit with her through a watch and let her talk the wreck back into being. She gives you the bearing she has been holding since the lights went out, and asks nothing for it but the passage.',
          effects: {
            flags: [
              { name: 'explore.cold-berth.resolved', value: 'heard' },
              { name: 'explore.cold-berth.bearing', value: true },
            ],
            disposition: [{ npcId: 'npc-doc-salvage', delta: 1 }],
          },
        },
        {
          id: 'put-her-ashore',
          label: 'Put her ashore at the next port',
          prose:
            'Log the rescue, feed her, and set her down where there are people. Whatever she is carrying is hers to carry; you have a marker of your own.',
          effects: {
            credits: 150,
            flags: [{ name: 'explore.cold-berth.resolved', value: 'ashore' }],
          },
        },
      ],
    },
    {
      id: 'explore.signal-debt.claim',
      title: 'A Claim Filed Against the Dark',
      prose:
        'The salvage claim off the buoy is properly filed, properly witnessed, and properly worthless: the ship it names never came back to enforce it. Properly worthless, that is, to anyone who cannot get to the coordinates on it.',
      repeat: 'never',
      trigger: {
        scheduledOnly: true,
      },
      wireResolution: {
        graceDays: 7,
        wireMessage:
          'The unenforced salvage claim off the buoy lapsed on its own schedule while it sat in your locker, and the registry struck it without comment.',
        effects: {
          flags: [{ name: 'explore.signal-debt.resolved', value: 'wire' }],
        },
      },
      choices: [
        {
          id: 'file-it-on',
          label: 'File it on in your own name',
          prose:
            'Walk it through the registry the slow, correct way. The clerk finds nothing to object to, which is the most surprising thing about the whole affair.',
          effects: {
            flags: [{ name: 'explore.signal-debt.resolved', value: 'filed' }],
            reputation: [{ faction: 'confederation', delta: 1 }],
          },
        },
        {
          id: 'sell-the-paper',
          label: 'Sell the paper on',
          prose:
            'Somebody always wants a claim on a wreck nobody has seen. Take the coin, hand over the page, and do not ask what they think is out there.',
          effects: {
            credits: 250,
            flags: [{ name: 'explore.signal-debt.resolved', value: 'sold' }],
            disposition: [{ npcId: 'npc-the-broker', delta: 1 }],
          },
        },
        {
          ...
          prose:
            'Some paper is a hook with a page attached. Put it through the recycler and let the dark keep whatever it took.',
          effects: {
            flags: [{ name: 'explore.signal-debt.resolved', value: 'burned' }],
          },
        },
      ],
    },
    {
      id: 'explore.black-ledger.courier',
      title: "The Courier's Rendezvous",
      prose:
        "The ledger off the wreck is a courier's, and a courier's ledger is mostly a calendar. The next line in it is a rendezvous, a set of numbers, and a countersign — and the date on it has not passed yet.",
      repeat: 'never',
      trigger: {
        scheduledOnly: true,
      },
      wireResolution: {
        graceDays: 5,
        wireMessage:
          "The rendezvous in the recovered courier's ledger came and went without you, and whoever was waiting for it did not wait twice.",
        effects: {
          flags: [{ name: 'explore.black-ledger.resolved', value: 'wire' }],
        },
      },
      choices: [
        {
          id: 'keep-the-appointment',
          label: 'Keep the appointment',
          prose:
            'Fly the numbers, speak the countersign, and take delivery of a package addressed to a dead man. Whoever hands it over is glad to be rid of it, and goes without looking back.',
          effects: {
            credits: 300,
            flags: [{ name: 'explore.black-ledger.resolved', value: 'kept' }],
            disposition: [{ npcId: 'npc-smuggler-ray', delta: 1 }],
          },
        },
        {
          id: 'turn-it-in',
          label: 'Turn the ledger in',
          prose:
            'Hand the whole book to the first patrol office that will take it. They log it, thank you flatly, and a rendezvous somewhere goes quietly wrong for somebody.',
          effects: {
            flags: [{ name: 'explore.black-ledger.resolved', value: 'turned-in' }],
            reputation: [{ faction: 'confederation', delta: 1 }],
          },
        },
        {
          id: 'let-it-lapse',
          label: 'Let the date pass',
          prose:
            'Close the book and put it in the locker. Nobody is owed an explanation by a captain who never opened it.',
          effects: {
            flags: [{ name: 'explore.black-ledger.resolved', value: 'lapsed' }],
          },
        },
      ],
    },
