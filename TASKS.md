...
### T-156 · Spec + owner ruling: N13 dawn-hand parity, (a) vs (b) — `status: TODO` · `coder: opus` · `after: T-130` · `[BLOCKED BY = Owner ruling: N13 hand design]`
`NPC_REDESIGN.md:1471-1497` (N13) has an owner ruling that a decision surface is required and an algorithmic fast-forward is *acceptable* — but the specific choice between (a) a literal reduced hand (2-3 dice/day, real per-day cost, makes `Crew`/`Reroll` real NPC verbs) and (b) an algorithmic equivalent (envelope-cheap, a flagged model rather than the decision, rules `Crew`/`Reroll` player-only) is explicitly recorded as "the owner's, made at step start and recorded here — not drifted into." Automated preparation: write up both options against the current codebase (confirm whether N12/Port has landed, since N13 is next in the doc's own run order after it; measure or estimate (a)'s per-day cost against the ~40ms envelope named in the doc), with a recommendation, mirroring T-151's bakeoff-and-spec pattern. Do not implement either design. Commit the spec, set status `BLOCKED(Owner ruling: N13 hand design)`, and halt.
**Accept:** (human-checked) a spec is committed laying out (a) and (b) with the envelope-cost question answered concretely, not deferred; the run halts with this task `BLOCKED`, never self-approved; only the owner flips it to `DONE` once the choice is made.
