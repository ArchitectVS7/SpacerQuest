...
### T-1001 · CI runs the whole gate — `status: TODO` · `coder: opus` · `after: —`
...
**Accept:** CI shows a green Playwright job executing all 11 existing spec files; PR notes demonstrate a deliberately broken spec turns CI red; a type error injected into an e2e spec fails `npx tsc -b` locally.
...
**Accept:** PR notes demonstrate each rewritten test fails under a seeded mutation of the rule it guards; a scripted broke-and-dry sim registers `fuelStarvationDays > 0`; the golden fixture is committed.
