...
### T-151 · Bakeoff + spec: player-modifying trinkets — `status: DONE` · `coder: opus` · `after: T-130`
...
**Delivered (2026-08-01):** `docs/PLAYER-TRINKETS_SPEC.md` — the bakeoff report and the settled
spec, reported in the skill's own six-part order. **Four independent reviewers** (systems
programmer, economy/progression designer, balance analyst, QA lead), same prompt, isolated
context, no shared state, plus a **full-engine rig**: eight variants generated by anchored
substitution on the shipped `state.ts:147-153` starting-stats literal, each anchor asserted to
match exactly once, the no-change control proven **byte-identical to production** (9/9 policy ×
seed full-JSON hashes), 300 seeds × 8 policies × 35 days per variant (n=2,400 rows each). **The
verdict is UNANIMOUS 4/4 for candidate C — do not build**, and the rig does not overturn it. The
spec nevertheless settles candidate A in full, with no open question left for an implementer, in
case the owner overrules: **no slot** (a one-time grant consumed into `player.stats`, forfeited on
death — the mandatory succession answer, since `applySuccession` today RESETS the ship but CARRIES
stats, so silence would make a trinket the only acquired bonus that outlives its owner), **`+1` as
a literal type** so `+2` is a compile error, **PILOT and TRADE only — GUNS, GRIT and GUILE are not
members of the proposed `TrinketStat` type**, **the Explore unique-item row at band 4 only**, and
**no save-shape change — `CURRENT_SAVE_VERSION` stays 15** (§8; the task block's inherited "12" is
stale). **The rig overturned this spec's own pre-measurement draft**, which had put GUNS in scope
because no *collision* could be found for it: at n=2,400 a +1 GUNS changes 2.5% of campaigns and
beats the control on 1 of 8 policies by 15 credits, and a +1 GUILE changes **2 campaigns out of
2,400** — both are measured dead options, and the spec was rewritten to follow the data. **The
measured case for C is stronger than "no effect":** the one stat that genuinely works is GRIT,
which cuts ship losses **20% at +1 and 46% at +2** — landing entirely on the curve
`content/components.ts` ratifies as *"held where it stands"* — while the two stats that may safely
be touched buy a naive-choice regret of only 174–413cr against day-35 medians of 1,120–21,418. **Five framing
corrections, three of which change the answer:** the task's own `±1-2` "mirroring ship-component
deltas" premise is false twice over (component deltas are damped 10:1 by `NAV_BONUS_DIVISOR` and
feed one reader; a stat delta is undamped and feeds every reader — `navigation +10` is the *first*
delta in the whole content table that buys a whole `+1`, and it is one of only 5 band-4 items);
`applyUniqueItem` dispatches on `item.class` with `if`/`else`, **not** an exhaustive switch, so a
third arm would fall through silently (F-151-3); and a new content file does **not** fail
`balance-rig.test.ts` — content is hashed wholesale (F-151-4). **The exact solve found an
inversion no individual reviewer made:** there are two check families, and `+1` is worth the full
+5pp only for GRIT (read off rolls the player does *not* choose), against **+1.28pp** for every
spent-die stat, where a 5-die hand already clears DC 12 at 96.88% — so a uniform band across five
stats is wrong by ~4× *between* stats, and most generous exactly where the game has ratified a
band. **Ten findings filed** (F-151-1 … F-151-10) — including **F-151-9, an INSTRUMENT finding:
the `fighter` policy's day-35 median is 2,825cr in all eight variants, bit-for-bit flat under every
stat change including +2 GRIT, so the rig cannot separate "GUNS is dead" from "the instrument
cannot see GUNS"** (which is why §5.3 argues the GUNS exclusion on two independent legs) — and the
arithmetic error all four
reviewers found independently at `content/components.ts:164` (the GRIT-1 life-support save is
**60%**, not the documented 55% — that is the GRIT-0 figure) — **filed, not fixed**, since T-151
touches no content source. **Zero engine, content, UI or sim source lines; no fingerprint, band,
threshold or golden edited; no capstone taken** (a markdown-only change moves nothing —
`computeDocsFingerprint` hashes TypeScript bytes, not `docs/`). `git diff --stat` shows exactly
two paths: this file and the new spec. **The go/no-go, the ruling-2 question, and a third fresh
call the task did not ask for — whether to break the write-once contract on `player.stats` — are
handed back UNRULED in §12.** Orchestration: graphify=none — no `graphify-out/graph.json` in the repo root (checked; absent), so I oriented by reading `docs/EXPLORE_REDESIGN.md` §4/§7, `docs/HANGOUT_REDESIGN.md` he · attempts=1/4.
