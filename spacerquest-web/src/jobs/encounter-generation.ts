 ...
 */
export async function runEncounterJob(): Promise<EncounterJobResult> {
  const result: EncounterJobResult = {
    botCombats: 0,
    takeoverAttempts: 0,
 ...
 */
async function generateBotCombats(): Promise<number> {
 ...
 */
async function processTakeoverAttempts(): Promise<{ attempts: number; successes: number }> {
  ...
  for (const system of allianceSystems) {
    ...
    if (!checkProbability(0.01)) continue;
    ...
    if (system.lastTakeoverAttempt) {
      const daysSinceAttempt = Math.floor(
        (Date.now() - system.lastTakeoverAttempt.getTime()) / (1000 * 60 * 60 * 24)
      );
      
      if (daysSinceAttempt < 7) continue; // 7 day cooldown
    }
    
    // Pick a random rival alliance
    const alliances = Object.values(AllianceType).filter(
      a => a !== AllianceType.NONE && a !== system.alliance
    );
    
    if (alliances.length === 0) continue;
 ...
 */
async function updateFuelPrices(): Promise<number> {
