...
export async function upgradeShipComponent(
  ...
  if (upgradeType === 'STRENGTH') {
    updateData[strengthField] = Number(character.ship[strengthField as keyof typeof character.ship]) + 10;
  } else {
    updateData[conditionField] = Math.min(9, Number(character.ship[conditionField as keyof typeof character.ship]) + 1);
  }
