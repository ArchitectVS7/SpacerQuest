...
export async function registerEconomyRoutes(fastify) {
    // Buy fuel
    ...
    fastify.post('/api/economy/cargo/accept', {
        ...
        const contract = generateCargoContract(character.currentSystem, character.ship.cargoPods, false);
        await prisma.character.update({
            where: { id: character.id },
            data: {
    ...
    fastify.post('/api/economy/cargo/deliver', {
        ...
        const { payment, bonus, total, message } = calculateCargoPayment(contract, character.currentSystem);
    ...
    fastify.post('/api/economy/alliance/invest', {
        ...
        const { userId } = request.user;
        const { amount, type, systemId, levels } = request.body;
