...
export async function investInAlliance(characterId, amount) {
...
export async function withdrawFromAlliance(characterId, amount) {
    const character = await prisma.character.findUnique({
        where: { id: characterId },
    });
    if (!character) {
    ...
    const { success: canWithdraw, high: newInvHigh, low: newInvLow } = subtractCredits(membership.creditsHigh, membership.creditsLow, amount);
    if (!canWithdraw) {
        return { success: false, error: 'Not enough invested credits' };
    }
    const { high: newCharHigh, low: newCharLow } = addCredits(character.creditsHigh, character.creditsLow, amount);
    await prisma.$transaction([
        prisma.character.update({
            where: { id: characterId },
...
export async function investInDefcon(characterId, systemId, levels) {
    ...
    const { success: canAfford, high, low } = subtractCredits(character.creditsHigh, character.creditsLow, cost);
    if (!canAfford) {
