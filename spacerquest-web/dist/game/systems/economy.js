...
import { CARGO_BASE_RATES, CARGO_WRONG_DESTINATION_PENALTY, FUEL_DEFAULT_PRICE, FUEL_PRICES_BY_SYSTEM, FUEL_SELL_MULTIPLIER, PORT_BASE_PRICE, PORT_RESALE_MULTIPLIER, PATROL_BASE_PAY, PATROL_BATTLE_BONUS, } from '../constants';
 ...
 */
export function getFuelPrice(systemId, portOwnerPrice) {
    ...
    if (systemId in FUEL_PRICES_BY_SYSTEM) {
        return FUEL_PRICES_BY_SYSTEM[systemId];
    }
 ...
 */
export function calculateFuelBuyCost(units, pricePerUnit) {
 ...
 */
export function calculateFuelSaleProceeds(units, buyPrice) {
    return Math.floor(units * buyPrice * FUEL_SELL_MULTIPLIER);
}
/**
 * Get cargo description by type
 */
export function getCargoDescription(type) {
    const descriptions = {
        1: 'Titanium Ore',
        2: 'Capellan Herbals',
        3: 'Raw Dilithium',
 ...
 */
export function generateCargoContract(originSystem, cargoPods, isSmuggling = false) {
    // Determine cargo type based on origin
    let cargoType = 1;
    let baseRate = Number(CARGO_BASE_RATES[1]);
 ...
 */
export function calculateCargoPayment(contract, actualDestination) {
 ...
 */
export function calculatePortPrice(systemId) {
 ...
 */
export function calculatePortResaleValue(purchasePrice) {
    return Math.floor(purchasePrice * PORT_RESALE_MULTIPLIER);
}
/**
 * Purchase space port
 */
export async function purchasePort(characterId, systemId, creditsHigh, creditsLow) {
 ...
 * Sell space port
 */
export async function sellPort(characterId, systemId, creditsHigh, creditsLow) {
 ...
 */
export function calculateLandingFee(hullStrength, systemId) {
 ...
 */
export async function collectPortFees(portId, characterId, hullStrength, systemId) {
    ...
    if (!port)
        return 0;
    const { high: newHigh, low: newLow } = addCredits(port.bankCreditsHigh, port.bankCreditsLow, fee);
 ...
 */
export function calculatePatrolPay(battlesWon) {
    return PATROL_BASE_PAY + (battlesWon * PATROL_BATTLE_BONUS);
}
/**
 * Process patrol mission completion
 */
export async function completePatrolMission(characterId, battlesWon) {
    ...
    // Add credits
    const { high: newHigh, low: newLow } = addCredits(character.creditsHigh, character.creditsLow, pay);
    await prisma.character.update({
        where: { id: characterId },
        data: {
            creditsHigh: newHigh,
 ...
 */
export async function processRescuePayment(rescuerCharacterId) {
    ...
    // Add credits
    const { high: newHigh, low: newLow } = addCredits(character.creditsHigh, character.creditsLow, fee);
    await prisma.character.update({
        where: { id: rescuerCharacterId },
        data: {
            creditsHigh: newHigh,
