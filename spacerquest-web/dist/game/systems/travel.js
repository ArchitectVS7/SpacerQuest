 ...
 */
import { COURSE_CHANGE_FUEL_MULTIPLIER, TRAVEL_TIME_MULTIPLIER, DAILY_TRIP_LIMIT, } from '../constants';
 ...
 */
export function calculateFuelCost(driveStrength, driveCondition, distance) {
 ...
 */
export function calculateCourseChangeFuel(hullStrength) {
 ...
 */
export function calculateFuelCapacity(hullStrength, hullCondition) {
    if (hullStrength < 1 || hullCondition < 1)
        return 0;
 ...
 */
export function hasEnoughFuel(currentFuel, requiredFuel) {
 ...
 */
export function calculateArrivalTime(departureTime, distance) {
 ...
 */
export async function validateLaunch(characterId, destinationSystemId) {
    const errors = [];
    const warnings = [];
 ...
 *   f1=f1-x
 */
export async function processCourseChange(characterId, newDestinationId, courseChangesRemaining) {
    const { prisma } = await import('../../db/prisma.js');
    const character = await prisma.character.findUnique({
        where: { id: characterId },
        include: { ship: true },
 ...
 */
export async function startTravel(characterId, originSystem, destinationSystem, fuelReserved) {
    const { prisma } = await import('../../db/prisma.js');
    const now = new Date();
 ...
 */
export async function completeTravel(characterId, destinationSystem) {
    const { prisma } = await import('../../db/prisma.js');
    // Remove travel state
 ...
 */
export async function getTravelProgress(characterId) {
    const { prisma } = await import('../../db/prisma.js');
 ...
 */
export async function registerLostInSpace(characterId, location) {
    const { prisma } = await import('../../db/prisma.js');
    await prisma.character.update({
        where: { id: characterId },
        data: {
