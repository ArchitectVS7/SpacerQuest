...
export async function registerWebSocketHandler(fastify) {
    fastify.get('/ws', { websocket: true }, async (connection, req) => {
        ...
        socket.on('combat:action', async (data) => {
            if (!socket.characterId || !socket.userId)
                return;
            ...
            if (data.action === 'RETREAT') {
                const retreat = attemptRetreat(character.ship.driveStrength * character.ship.driveCondition, data.enemy?.driveStrength * data.enemy?.driveCondition || 100, character.ship.hasCloaker);
            ...
            const combatRound = processCombatRound(playerBF, character.ship.weaponStrength, character.ship.weaponCondition, character.ship.shieldStrength, character.ship.shieldCondition, data.enemy || {
                weaponStrength: 20,
                weaponCondition: 7,
                shieldStrength: 15,
                shieldCondition: 7,
        ...
        socket.on('screen:request', async (data) => {
            if (!socket.characterId)
                return;
        ...
        // Handle screen input
        socket.on('screen:input', async (data) => {
            if (!socket.characterId)
                return;
