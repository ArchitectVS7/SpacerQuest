...
export function App() {
    const { isAuthenticated, token, character, currentScreen, setAuthenticated, setCurrentScreen, } = useGameStore();
    ...
    const checkCharacterStatus = async (authToken) => {
        try {
    ...
    useEffect(() => {
        ...
        const handleTravelComplete = (data) => {
    ...
    // Loading state
    if (!hasCheckedAuth) {
        return (_jsx("div", { className: "min-h-screen bg-black text-green-500 flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx("p", { className: "text-xl mb-4", children: "SpacerQuest v4.0" }), _jsx("p", { className: "animate-pulse", children: "Loading..." })] }) }));
    }
    ...
    if (needsCharacter || currentScreen === 'character-create') {
        return _jsx(CharacterCreationScreen, {});
    }
    // Main game terminal
    return _jsx(TerminalComponent, {});
}
export default App;
