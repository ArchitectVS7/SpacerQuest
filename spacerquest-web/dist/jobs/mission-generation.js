...
import { NEMESIS_REQUIREMENT_WINS, PATROL_BASE_PAY, } from '../game/constants.js';
 ...
 */
async function generatePatrolMissions() {
 ...
 */
async function checkNemesisEligibility() {
    const eligibleCharacters = await prisma.character.findMany({
        where: {
    ...
    for (const char of eligibleCharacters) {
        ...
        if (char.cargoManifest?.includes('Nemesis'))
            continue;
 ...
 * Generate special events
 */
async function generateSpecialEvents() {
    let events = 0;
...
//# sourceMappingURL=mission-generation.js.map